...
const SUfemalehostel = () => {
    ...
    const handleEditTask = (task: { id: number; hostel: string; status: string }) => {
        setEditingTask(task.id);
        setEditingName(task.hostel);
    ...
    const initialUsers = [
        { id: 1, name: 'Fat Spaghetti', hostel: 'Old Girl', status: 'A121' },
        { id: 2, name: 'Space Dad', hostel: 'Old Girl', status: 'A121' },
    ];

    const [users, setUsers] = useState(initialUsers);
    const [newUser, setNewUser] = useState({ name: '', hostel: '', status: '' });
    const [editingId, setEditingId] = useState<number | null>(null);
    const [editUser, setEditUser] = useState({ name: '', hostel: '', status: '' });
    ...
    const addRow = () => {
        if (!newUser.name.trim() || !newUser.hostel.trim() || !newUser.status.trim()) return;
        setUsers([...users, { id: users.length + 1, ...newUser }]);
        setNewUser({ name: '', hostel: '', status: '' });
    };

    const editRow = (user: { id: number; name: string; hostel: string; status: string }) => {
        setEditingId(user.id);
        setEditUser({ name: user.name, hostel: user.hostel, status: user.status });
    };
    ...
    const saveRow = (id: number) => {
        ...
        setEditingId(null);
        setEditUser({ name: '', hostel: '', status: '' });
    };
            ...
            <tr key={user.id}>
                {editingId === user.id ? (
                    <>
                        ...
                        <td className="py-3 px-2"><input className="bg-white/5 rounded-md text-white w-20" value={editUser.hostel} onChange={(e) => handleInputChange(e, 'hostel')} /></td>
                    ...
                    <>
                        ...
                        <td className="py-3 px-2">{user.hostel}</td>
    ...
    return (
        <div className="bg-white/10 col-span-9 rounded-lg p-6">
            <h1 className="font-bold py-4 uppercase">SU Female Hostel List</h1>
                ...
                <table className="w-full whitespace-nowrap">
                    ...
                    <thead className="bg-black/60 text-white">
                        <tr>
                            ...
                            <th className="text-left py-3 px-2" scope="col">Hostel</th>
                            <th className="text-left py-3 px-2" scope="col">Room</th>
            ...
            <div className="mt-4">
                <input className="bg-white/5 rounded-md text-white w-30 mr-2" value={newUser.name} onChange={(e) => handleInputChange(e, 'name')} placeholder="Name" />
                <input className="bg-white/5 rounded-md text-white w-20 mr-2" value={newUser.hostel} onChange={(e) => handleInputChange(e, 'hostel')} placeholder="Hostel" />
                <input className="bg-white/5 rounded-md text-white w-20 mr-2" value={newUser.status} onChange={(e) => handleInputChange(e, 'status')} placeholder="Room" />
